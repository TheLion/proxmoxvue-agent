...
type SupabaseConfig struct {
	// BaseURL is the Supabase host with scheme, e.g.
	// "https://fjesjyoxpkalaudfyebx.supabase.co" or
	// "https://api.proxmoxvue.app". Populated automatically from
	// ProjectRef on load if missing — see EnsureSupabaseDefaults.
	BaseURL string `yaml:"base_url,omitempty"`

	// ProjectRef is the legacy short identifier. Deprecated in favour of
	// BaseURL but kept for older config.yml files. Will be removed in
	// agent v2 after the sunset window.
...
func (c SupabaseConfig) String() string {
	...
	return fmt.Sprintf("{BaseURL:%s ProjectRef:%s ClusterID:%s RefreshToken:%s PrivateKey:%s}",
		c.BaseURL, c.ProjectRef, c.ClusterID, maskedToken, maskedKey)
}

// EnsureSupabaseDefaults migrates legacy ProjectRef-only configs to the
// BaseURL form. Idempotent: a no-op when BaseURL is already populated.
// Call once after Load(), before passing the config to clients that
// expect BaseURL.
func EnsureSupabaseDefaults(c *File) {
	if c.Supabase.BaseURL == "" && c.Supabase.ProjectRef != "" {
		c.Supabase.BaseURL = fmt.Sprintf("https://%s.supabase.co", c.Supabase.ProjectRef)
	}
}
...
var fieldHeadComments = map[string]string{
	...
	"supabase.base_url":           "Supabase host with scheme, e.g. https://<ref>.supabase.co. Resolved from project_ref if absent.",
	"supabase.project_ref":        "Supabase project ref. Set by --register. Legacy field; base_url takes precedence when set.",
